...
const CompanionList = ({title, companions, classNames }: CompanionListProps )=> {
  return (
    <article className={cn(  'companion-list' , classNames)}>
      <h2 className="font-bold text-3xl">Recent Seesions </h2>

      <Table>
        <TableHeader>
          <TableRow>
            <TableHead className="text-lg w-2/3">Leasons</TableHead>
        ...
        <TableBody>
          {companions?.map(({id, subject , name , topic , duration }) => (
            <TableRow key={id}>
              ...
              <TableCell >
                <div className="flex items-center justify-end gap-2 w-full justify-end">
                  <p className="text-2xl">{duration}  {' '} <span className="max-md:hidden ">mins</span></p>
                  <Image src="/icons/clock.svg" className="md:hidden" alt="minutes" width={13.5} height={13.5} />
